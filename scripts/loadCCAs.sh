...
echo -e "Available congestion control algorithms: ${BLD}$available_cca${RST}"

# Display currently allowed congestion control algorithms
allowed_cca=$(sysctl -n net.ipv4.tcp_allowed_congestion_control)
echo -e "Allowed congestion control algorithms: ${BLD}$allowed_cca${RST}"
...
if [ -n "$module_cca" ]; then
    echo -e "Found the module of the congestion control algorithms: ${BLD}$module_cca${RST}"
else
...
for cca in "${algos[@]}"; do
    ...
    echo -n "Loading congestion control algorithm: $cca... "
