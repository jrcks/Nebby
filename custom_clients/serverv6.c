...
void check_file_exist(const char *filename)
{
    ...
    {
        printf("The stats file %s already exists\n", filename);
    }
}
...
int main(int argc, char *argv[])
{
    ...
    servaddr.sin6_addr = in6addr_any; // Accept connections on any available network interface
    servaddr.sin6_port = htons(PORT);              // Convert port number to network byte order
